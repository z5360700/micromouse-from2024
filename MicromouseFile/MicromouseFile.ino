...
// const char commandSequence[] = "ssssssrrssssss";
// const char commandSequence[] = "fffrrfffll";
// const char commandSequence[] = "ffrflfrflflffrfrfflfflfffrrfffrffrfflflffrfffff";
// const char commandSequence[] = "frffflsrslssrssslssssrssssfrflffrff";
const char commandSequence[] = "frffsslsrslssrssslsssrssssfrflfflff";
...
void loop() {
  if (!isMoving) {
    if (commandIndex < sizeof(commandSequence) - 1) {
      ...
      displayMessage("ROBOT FINISHED");
      while (1); // Stop the robot
    }
  }

  if (isMoving) {
    // Call the appropriate movement function based on the current command
    char currentCommand = commandSequence[commandIndex - 1];
    if (currentCommand == 'f') {
      moveRobotForward();
...
void moveRobotForwardSmall() {
  while (true) {
    ...
    motor.setPWMRight(47.1 + wallFollowAdjustmentRight);
